...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}">
		...
		<!-- jQuery -->
		<script type="application/javascript" src="{{ asset('assets/jquery/jquery-3.5.1.min.js') }}"></script>
		...
		<div class="ui main container" id="app">
			<div class="ui one column grid">
				...
				<div class="column">
					...
					<form class="ui grid form m-0" method="post" action="{{ route('settings.install_app') }}" enctype="multipart/form-data">
						@csrf
						<div class="ui fluid card">
							...
							<div class="content footer ">
								<button class="ui d-none  large previeus button ml-0 mr-auto" @click="navigateSteps(-1)" type="button" :class="{disabled: stepIsActive(1)}">{{ __('Previous') }}</button>
								<button class="ui large next button ml-auto mr-0" @click="navigateSteps(1)" type="button"  v-if="step <= 2">{{ __('Next') }}</button>
								<button class="ui large yellow button ml-auto mr-0" type="button" @click="submitForm" v-if="step == 3" >{{ __('Submit') }}</button>
							</div>
						</div>
